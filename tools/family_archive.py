...
PAGES = [
    ...
    # Second from the top, above the browsing sections, because the reports are
    # the thing this archive is FOR from an examiner's seat — the statements that
    # leave the screen and get handed to someone. Examiner-only: they are
    # generated by the review and speak in review vocabulary, so a family session
    # sees neither the link nor the route.
    ("reports", "Reports", "heirloom"),
...
EXAMINER_ONLY = {"review", "history", "guided", "junk", "collections", "reports"}
...
class ArchiveCase:
    ...
    def section(self, page):
        ...
        if page == "reports":
            # The index. Which report to render is chosen by ?r= at the API layer
            # (see api_section) — this is only the list of what exists.
            return {"reports": [
                {"key": "estate", "label": "Estate document report",
                 "note": "For an attorney: which documents an estate needs are "
                         "present, which are not established, and what the search "
                         "could not reach."},
    ...
    def api_section(self, name, params=None):
        ...
        if name == "reports":
            # ?r= picks the report. Each is built here rather than in section() so
            # only the one being read is computed — the family report assembles
            # half a dozen other sections and is not free.
            which = _one(params, "r")
            when = datetime.now().strftime("%-d %B %Y")
            if which == "estate":
                # Built from the SAME vital_docs the Documents page reads, so the
                # report and the screen it came from can never disagree.
                return estate_report_data(self.section("documents")["vital_docs"],
                                          case_id=self.paths.case_id,
                                          generated_at=when)
            if which == "family":
                ov = self.section("overview")
                docs = self.section("documents")
                emails = apply_curation(self.section("emails"),
                                        self.curation_layer, "thread_id")
                return family_report_data(
                    counts=ov.get("counts") or {},
                    scene_counts=ov.get("scene_counts") or {},
                    audio_rows_=self.section("recordings"),
                    document_index=docs.get("index") or [],
                    email_categories=_email_facets(
